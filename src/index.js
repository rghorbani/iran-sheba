...
const pattern_code = /IR[0-9]{2}([0-9]{3})[0-9]{19}/;
 ...
 * @property {string} persianName
 * @property {string} code
 * @property {boolean} accountNumberAvailable
 * @property {string} [accountNumber]
 * @property {string} [formattedAccountNumber]
 */
...
function recognize(str) {
  ...
  const res = pattern_code.exec(str);
  const code = res[1];
  if (!banksHash[code]) {
    return false;
  }

  const bank = banksHash[code];
  const result = {
...
export { recognize, isValid, banksOutput as banks };
